...
void map_reader() {
	...
	for (size_t i = 0; i != smatrix.size(); ++i) { //удаление пробелов
	...
	for (size_t i = 1; i + 1 != matrix.size(); ++i) { // перенос строчной карты в основную
		for (size_t j = 1; j + 1 != matrix[i].size(); ++j) {
			size_t x = i - 1;
			size_t y = j - 1;
		...
		for (size_t j = 1; j + 1 != matrix[i].size(); ++j) {
			if (matrix[i][j] == 1) {
				...
				for (auto shift : Shifts) {
					size_t x = i, y = j;
					if (x + shift.x == matrix.size() - 1) x = 0;
					if (x + shift.x == 0) x = matrix.size() - 1;
					if (y + shift.y == matrix.size() - 1) y = 0;
					if (y + shift.y == 0) y = matrix.size() - 1;

					auto& cell = matrix[x + shift.x][y + shift.y];
					if (cell == 1) counter_neighbours++;
				}
				if (counter_neighbours < 2 && counter_neighbours > 3) {
					buffer_matrix[i][j] = death;
				}
				if (counter_neighbours == 2 || counter_neighbours == 3) buffer_matrix[i][j] = life;
			}
